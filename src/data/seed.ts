import type { AppData, CropType } from '../domain/types';
...
const txs: SeedTx[] = [
  { d: 0, hour: 7, supplierId: 'sup-1', lines: [line('rubber', 320, 5, 14500)], amountPaid: 4000000 },
  {
    d: 0,
    hour: 8,
    supplierId: 'sup-2',
  ...
  { d: 1, hour: 14, supplierId: 'sup-1', lines: [line('rubber', 280, 6, 14500)], amountPaid: 0 },
  {
    d: 2,
    hour: 8,
    supplierId: 'sup-5',
...
export const buildSeedData = (): AppData => ({
  ...
  transactions: txs.map((t, i) => {
    ...
    return {
      ...
      lines: t.lines.map((l, j) => ({ ...l, id: `seed-${i + 1}-line-${j}` })),
